...
- `tools/issue_io` and `tools/repo_io` are the GitHub-owning modules — `issue_io` files/reads human-escalation issues, `repo_io` is the repo/branch/PR factory, now **five** verbs (`create_repository`, `clone_repo`, `create_branch`, `open_pr`, and — sprint 36, BL-21 — `create_ruleset`; no merge verb — auto-merge is prohibited) **plus repo introspection** (`resolve_repo_slug`, Sprint 27 follow-up: the `owner/repo` a tree belongs to). `resolve_repo_slug` lives here, not in `issue_io`, because it is a repo verb — but its caller is the issue filer, which needs an **explicit** destination rather than `gh`'s implicit CWD resolution (finding R8). It is orchestrator-side and deliberately **not** an MCP verb: the github server's four-verb set stays pinned and pairwise-disjoint — `create_ruleset` follows `resolve_repo_slug`'s precedent of an orchestrator-only `repo_io` verb that never enters `mcp_servers/github_server`. Both shell out to the already-authenticated `gh`; no other module talks to GitHub.
...
- `flows/bootstrap` (Phase 5 piece 4; ruleset step added sprint 36, BL-21) is a sibling of `flows/maintenance` — the factory verb that brings a new, conventions-conformant repo into *existence*: `repo_io.create_repository` (`private=False` by default — BL-21: a ruleset is only installable on a public repo under the org's Free plan, so protection is the default and privacy is an opt-in that knowingly forfeits it) → `repo_io.clone_repo` (empty tree) → `git_io.checkout_branch(main)` → `tools/scaffold.write_skeleton` → `git_io.commit_all`/`push_branch(main)` → `repo_io.create_branch(develop, base=main)` (ordering is load-bearing — `create_branch` reads the base ref's SHA over the API, so it runs only *after* the push) → `repo_io.create_ruleset(main, develop)`, run **last** and skipped outright when `private=True` (a `pull_request` rule on `main` installed any earlier would reject the scaffold's own initial push — FD7); a live `create_ruleset` failure returns `BootstrapStatus.RULESET_FAILED` with the `RepoRef` intact rather than propagating, so a caller can still tear down the (unprotected) repo. Deliberately **no** inner loop (`run_in_tree`), **no** green gate, and **no** `open_pr` — a brand-new repo has nothing to PR into; auto-merge stays impossible. Same boundary posture as `flows/maintenance`: no `keyring`, no direct file write, no subprocess surface of its own (asserted by `tests/flows/test_boundaries.py`).
