...
> posted against its **current head commit** (`.github/workflows/hitl-review.yml`).
...
- `core/` imports no concrete persona module, only `personas/base.py`. It is otherwise unrestricted on `tools/*`, and two `core/` modules now reach `tools/mcp`: `core/coder_gate.py` (Sprint 28) imports it directly, and `core/engine.py` reaches it **transitively** via `tools/issue_io` (whose `mcp_client` builds the issue provider) since the Sprint-27 issue-path flip — so importing `core.engine` pulls the MCP client stack at import time. `core/coder_gate.py`'s direct import exists so the `RalphCoderGate`'s evidence pytest run can dispatch through `tools/mcp.run_gate_pytest` (in-process on `none`/`worktree`, the sandboxed coder-tools provider on `container`/`sandbox`) instead of refusing under sandbox modes as `_raise_if_sandboxed` (deleted) once did.
