...
- `tools/slack_io` (Sprint 39, BL-2 pass 1 of 3) is the **notifier transport** — `slack_sdk` is the only module that imports it, and even there the import is **function-scoped inside `SlackNotifier.emit`** (not module scope), so `build_notifier_from_env()`'s no-op default (either env var unset) never pulls it into the import graph — the same F7 posture as `tools/issue_io/mcp_client.py`'s deferred `tools/mcp` import. `core/notify.py` is the pure, leaf `EventKind`/`LifecycleEvent`/`Notifier`/`NoOpNotifier` contract (no `slack_sdk` import); `core/graph_engine.py`'s `run_graph_loop` resolves a notifier via `build_notifier_from_env()` and emits lifecycle events through that seam. Emission is **fail-open at the call site** (`_safe_emit`): a formatter bug, bad token, network error, or Slack outage is caught and swallowed, never propagated — a notification failure must never fail a run. Credentials are `LOOP_ENGINE_SLACK_BOT_TOKEN`/`LOOP_ENGINE_SLACK_CHANNEL`, **env vars, not keyring** (FD3) — a distinct credential class from the keyring-only Anthropic key, same posture as `trigger/`'s `LOOP_ENGINE_WEBHOOK_SECRET`. Adds no subprocess surface, so the five sanctioned surfaces above are unchanged.
