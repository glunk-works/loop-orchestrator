...
- **`tools/mcp/`** — the runtime MCP client: `MCPToolProvider` connects to stdio servers on a background event-loop thread, discovers tools via `list_tools`, and exposes `(tools, execute)` to `run_tool_loop`. Selected by `LOOP_ENGINE_TOOLS=mcp`; the Coder's `_CoderToolBackend` opens one provider per run (lazily, on first tool-loop use) and closes it in a `finally`. Default stays the in-process `CODER_TOOLS`/`_execute_tool` dispatch. `build_coder_tool_provider` picks the server *launch params* by isolation mode (Phase 3b): `container` → `container_server_params` (docker/podman, mounts only the worktree), `sandbox` → `sandbox_server_params` (`bwrap`, daemon-free — secondary/inert), else the local `coder_tools_server_params`. Both sandbox builders raise `IsolationUnavailableError` when no runtime is present — the mode **never** silently degrades to in-process execution — and `container`/`sandbox` require `LOOP_ENGINE_TOOLS=mcp` (there is no server to sandbox on the in-process path). The container/sandbox is spawned by the same MCP `stdio_client` that launches the local server (only `command`/`args` change), so it adds **no new sanctioned subprocess surface**; runtime detection is `shutil.which`, not a subprocess. **Inert on this branch** (no runtime here; `unshare -Ur` is denied): argv-shape tested only — the real `docker run` e2e + the sandboxed *gate* pytest are deferred (`sprints/18_execution_isolation_container/sprint_plan.md`, `docs/phase3_execution_isolation_plan.md`).
