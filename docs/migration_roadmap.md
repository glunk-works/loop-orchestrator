...
**▶ NEXT ACTION: plan sprint 36 — live factory verification** (`DEFERRED_VERIFICATION.md` **§5** `github_server` verbs + **§7** maintenance flow + **§8** bootstrap flow, on a daemon-bearing host with authenticated `gh`, network, and one disposable scratch-repo lifecycle). These are the **only** checks with real side effects on GitHub, and together they decide whether **the factory actually works** — the product's central claim, and **still unverified against real GitHub after 25 sprints**. Every other guarantee in this repo is hermetic and says nothing about it.

> **⚠️ Read BL-21 before planning it.** `flows/bootstrap` ships repos with an **unprotected `main`** — a defect **in the very flow §8 exists to verify**. Verifying it as-built would confirm it *works*, which is **not** the same as confirming it is *right*. Sprint 36 should almost certainly **fix BL-21 and then verify**, not verify and then discover.
