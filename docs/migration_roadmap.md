...
**▶ NEXT ACTION: none pre-selected — the migration is fully closed and the next unit is a planning decision (Opus).** Sprint 32 (`artifact_refs` strip, FD3 superseded/inverted by FD1) is **complete, green, HITL-reviewed by Opus and approved (2026-07-12, PR #39, squash `3db3237`)**. With it, every migration task is discharged and no roadmap phase remains open. What is left is not migration work: (a) `sprints/DEFERRED_VERIFICATION.md` — five checks (§1, §5, §6, §7, §8) that have **never been run**; (b) `docs/backlog.md` BL-1…BL-10, of which **BL-10 is a live CI hole**, not an idea (a >72-char PR title silently skips the entire test suite and the checks page shows no red — logged in PR #40, `46458af`, deliberately not fixed); (c) two non-blocking findings from #39's review worth folding in before that code is load-bearing (`publish_artifacts` reads every artifact off disk on every stage while its docstrings claim it does no I/O; its new read-back uses `Path.read_text()` with no explicit encoding). `.ai/next-steps.md` is the live cursor.
