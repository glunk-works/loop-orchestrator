...
**▶ NEXT ACTION: sprint 35 (`35_migration_merge`) — land `feat/mcp-langgraph-migration` on `main` as its one-time merge commit.** Every roadmap phase has been closed since sprint 32 (`artifact_refs` strip: **complete, green, HITL-reviewed by Opus and approved**, 2026-07-12, PR #39, squash `3db3237`), but the migration's *code* being done is not the migration being *landed* — the entire thing has lived on an integration branch this whole time, and `main` is ~109 commits behind. Sprint 35 is that landing, planned and approved 2026-07-13 (PR #55, squash `3ee723e`); its plan (`sprints/35_migration_merge/sprint_plan.md`) holds seven locked decisions, FD1–FD7. The merge is **conflict-free and adds exactly one file** to the integration branch (`.github/workflows/ruleset-drift.yml`) — measured, not assumed — so the risk lives in **ordering and repo settings, not in code**: `allow_merge_commit` must stay enabled across the merge and be disabled straight after (BL-13), and Dependabot PRs #50–53 must not merge first (they would rewrite the exact `ci.yml` `uses:` SHA lines the clean merge depends on, and a conflicted PR here runs zero CI silently).

Folded into sprint 35 rather than left loose: the **two non-blocking findings from #39's review** (`publish_artifacts` reads every artifact off disk on every stage while its docstrings claim it does no I/O; its read-back uses `Path.read_text()` with no explicit encoding) are **Task 1**, landing on `feat` *before* the migration PR is cut. Still open and **not** blocking the merge (FD4): `sprints/DEFERRED_VERIFICATION.md`'s five never-run checks (§1, §5, §6, §7, §8), which sprint 35's Task 6 gives a scheduled home. `docs/backlog.md` BL-10 and BL-11 are **both resolved** (sprints 33 and 34) — BL-10 was the live CI hole where a >72-char PR title silently skipped the whole suite; BL-11 was the discovery that none of the "required" checks was enforcing anything at all. `.ai/next-steps.md` is the live cursor.
