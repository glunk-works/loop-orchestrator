...
**▶ NEXT ACTION: plan sprint 37 — the test-review block, front-loaded ahead of BL-2** (owner decision 2026-07-15, to speed sprint iteration velocity): **BL-22** (CI/runner-time velocity — session-scope the MCP-subprocess fixtures first as the safe win; design the docs-PR CI exemption as an aggregator job, **not** `paths-ignore`, which is the BL-10/BL-12 trap) → **BL-23** (test-validity audit) → **BL-2** (Slack control plane). Post-migration work is backlog-driven from `docs/backlog.md`; the migration itself is done.
