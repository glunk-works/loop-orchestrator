...
**▶ NEXT ACTION: plan BL-2 (Slack control plane).** `docs/backlog.md`'s BL-2 entry carries a `SCHEDULED` note (added 2026-07-14) with three open questions the planning pass must weigh before drafting a `sprint_plan.md`: **BL-24**'s never-verified inbound trigger surface, the Slack credential class (distinct from the keyring-only Anthropic key and the env-var webhook secret, per the `trigger/` precedent), and whether it's a new top-level orchestrator caller (sibling of `trigger/`/`flows/`) or an MCP server. Post-migration work is backlog-driven from `docs/backlog.md`; the migration itself is done. BL-23 is a multi-pass program — pass 2 (the BL-32 adversarial-guard audit, or extending mutation testing beyond `core/`) is a later beat, not blocking BL-2.
