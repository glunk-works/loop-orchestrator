...
**▶ NEXT ACTION: sprint 32 (`artifact_refs` strip, FD3 superseded/inverted by FD1) is implemented — open the PR and get the Opus HITL review in a fresh session (`.ai/next-steps.md` is the live cursor). After that lands, the only standing obligation left is `sprints/DEFERRED_VERIFICATION.md` (below).**
