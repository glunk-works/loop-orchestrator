...
**▶ NEXT ACTION: the deferred `State.artifacts` strip (decision FD3) as its own sprint — Phase 6 is closed and did NOT include it. Plan it first (Opus): it is a behavior-changing refactor, not a deletion, because the `artifacts` readers were always the personas and gates, not `run_loop`.**

> Phase 6 is done and the migration is collapsed to one path. Two standing obligations
> survive it, deliberately: **FD3** (the `artifacts` strip, above) and
> **`sprints/DEFERRED_VERIFICATION.md`** — five checks (§1, §5, §6, §7, §8) that have
> **never been run** because they need a real key, a real authenticated `gh`, or a
> daemon-bearing host that can bind a port. Neither is a migration task; both are real.
> A green `hatch run test` says nothing about either.
...
runtime config. **Task 5 (the `State.artifacts` strip) is deferred to its own sprint —
see decision FD3 below; it is a refactor, not a deletion, and does not block anything.**
