...
**▶ NEXT ACTION: plan BL-2 pass 3 of 3 (the escalation round-trip).** Route a paused run's questions to Slack and fold the reply back, as an alternative to the GitHub-issue round-trip — the last BL-2 pass, with both the notify (pass 1) and command (pass 2) directions live to build on. **BL-24 is now "superseded in practice" but explicitly NOT closed**: `trigger/`'s HMAC path remains unverified and Slack proves nothing about it (no shared code), so the open decision is **retire `trigger/` + `LOOP_ENGINE_WEBHOOK_SECRET` as moot, or keep it and pay for the §6 verification** — owner's call, see the BL-24 supersede note. Post-migration work is backlog-driven from `docs/backlog.md`; the migration itself is done. BL-23 is a multi-pass program — pass 2 (the BL-32 adversarial-guard audit, or extending mutation testing beyond `core/`) is a later beat, not blocking BL-2.
