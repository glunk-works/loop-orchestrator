...
**▶ NEXT ACTION: plan sprint 38 — BL-23 (test-validity audit).** Mutation-test the enforced boundary guards (`tests/test_subprocess_surfaces.py`, `tests/test_ci_config.py`, the `core/`↔`personas/` import rules, the MCP pairwise-disjoint verb-set assertions) and hunt orphan/weak tests — the sprint-37 velocity work makes the suite fast enough to run mutation passes affordably. Then **BL-2** (Slack control plane). Post-migration work is backlog-driven from `docs/backlog.md`; the migration itself is done.
