...
sprints 28 **and 29** (F-GATE-SANDBOX + F-TOOLLOOP-CAP + F-CODER-NO-LINT all closed
in code). V2 stays **OPEN** — the re-attempt run converged 11 tasks but hit
`BUDGET_EXCEEDED` one sprint short of `COMPLETED`; the remaining obligation is that
single terminal observation, gated only on **escalation-free staging** (a real
remote or an injected non-crashing `issue_filer`). V1 is PASS(qualified) for
`ENGINE`/`TOOLS`/`PERSONAS`. The flip block itself is planned (`sprints/27_phase6_flip_block/`): the
