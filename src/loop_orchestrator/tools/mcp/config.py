"""`loop_orchestrator.mcp.json` schema + loader — config-driven, consumer-scoped MCP
server discovery (Phase 5, cross-cutting #3).

Named `loop_orchestrator.mcp.json` (not `.mcp.json`) because repo-root `.mcp.json` is
already Claude Code's own project MCP config (a different schema/purpose — the
devcontainer's hosted-github wiring); loop-orchestrator owns a distinct, namespaced
...
_CONFIG_FILENAME = "loop_orchestrator.mcp.json"
...
# The github factory-verb server's logical name in loop_orchestrator.mcp.json — no
...
# The issue escalation-verb server's logical name in loop_orchestrator.mcp.json — same
...
class MCPServerSpec(BaseModel):
    """One logical server's static launch spec, as declared in `loop_orchestrator.mcp.json`.
...
class MCPConfigFile(BaseModel):
    """The `loop_orchestrator.mcp.json` top-level shape."""
...
def _repo_root() -> Path:
    ...
    `loop_orchestrator.mcp.json` resolution independent of `Path.cwd()`, which worktree
...
def load_mcp_config(path: str | Path | None = None) -> dict[str, MCPServerSpec]:
    """Load `loop_orchestrator.mcp.json` (repo root, unless `path` overrides — tests use this).
    ...
    for name, spec in parsed.servers.items():
        ...
        # the env that actually holds `loop_orchestrator`. An absolute path is left
