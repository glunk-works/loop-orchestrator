...
from loop_orchestrator.tools.isolation import IsolationUnavailableError, sandbox_runtime_mode
...
_CODER_TOOLS_SERVER_MODULE = "loop_orchestrator.mcp_servers.coder_tools_server"
...
_CONTAINER_RUNTIME_ENV_VAR = "LOOP_ORCHESTRATOR_CONTAINER_RUNTIME"
_DEV_IMAGE_ENV_VAR = "LOOP_ORCHESTRATOR_DEV_IMAGE"
_SANDBOX_UID_ENV_VAR = "LOOP_ORCHESTRATOR_SANDBOX_UID"
...
def _container_runtime() -> str:
    ...
    `LOOP_ORCHESTRATOR_CONTAINER_RUNTIME` override wins; otherwise the first one found
    ...
    override = getenv_compat(_CONTAINER_RUNTIME_ENV_VAR, "").strip().lower()
    if override:
...
def _sandbox_user() -> str:
    ...
    override = getenv_compat(_SANDBOX_UID_ENV_VAR, "").strip()
    if override:
        return override
...
def container_server_params(worktree: str | Path) -> StdioServerParameters:
    ...
    image = getenv_compat(_DEV_IMAGE_ENV_VAR, "").strip()
    if not image:
        raise IsolationUnavailableError(
...
def build_provider_for(names: list[str], *, cwd: str | Path | None = None) -> MCPToolProvider:
    ...
    `loop_orchestrator.mcp.json` (or its built-in default). Each consumer names only
    the servers it wants — the model's coder tool loop names `coder_tools`
    alone, so a `loop_orchestrator.mcp.json` declaring other servers (e.g. `github`)
    ...
    for name in names:
        if name not in config:
            raise KeyError(f"MCP server {name!r} is not declared in loop_orchestrator.mcp.json")
        spec = config[name]
...
def build_issue_provider() -> MCPToolProvider:
    """A provider scoped to the `issue` escalation-verb server alone — the
    orchestrator's own helper, structurally separate from
    `build_coder_tool_provider` so the model's tool loop (which names only
    `coder_tools`) can never reach these verbs. No `cwd` sandbox override:
    like github, issue is not launched per-worktree."""
    return build_provider_for([ISSUE_SERVER_NAME])
