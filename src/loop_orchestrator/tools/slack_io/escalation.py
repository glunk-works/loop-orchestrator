...
open questions to `LOOP_ORCHESTRATOR_SLACK_CHANNEL` and returns a `SlackRef`
...
_TOKEN_ENV = "LOOP_ORCHESTRATOR_SLACK_BOT_TOKEN"  # noqa: S105 -- env var name, not a credential
...
def render_question_message(state: State, questions: list[Question]) -> str:
    ...
    """
    lines = [
        f"loop-orchestrator run `{state.run_id}` is paused: {len(questions)} question(s) "
        "need a human answer.",
        "",
    ]
...
def slack_escalation_filer(state: State, questions: list[Question], snapshot_hint: str) -> SlackRef:
    ...
    Mode delivers in inbound events even when `LOOP_ORCHESTRATOR_SLACK_CHANNEL` is
    ...
    """
    token = getenv_compat(_TOKEN_ENV)
