...
The single reader of `LOOP_ORCHESTRATOR_ISOLATION`, shared by `tools/worktree` (which
...
imports no other `loop_orchestrator` module, so it is safe to import from anywhere.
...
_ISOLATION_ENV_VAR = "LOOP_ORCHESTRATOR_ISOLATION"
...
def isolation_mode() -> IsolationMode:
    ...
    raw = getenv_compat(_ISOLATION_ENV_VAR, "").strip().lower()
    if raw == "":
        return "none"
