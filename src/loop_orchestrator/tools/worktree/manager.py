...
Selected by `LOOP_ORCHESTRATOR_ISOLATION=worktree` (default off): when unset,
...
from loop_orchestrator.tools.isolation import worktree_needed
...
_WORKTREE_ROOT_ENV_VAR = "LOOP_ORCHESTRATOR_WORKTREE_ROOT"
...
# is loop-orchestrator's own remote — which is how real escalation issues for managed
...
def worktree_root() -> Path:
    """Base directory holding per-run worktrees. `LOOP_ORCHESTRATOR_WORKTREE_ROOT`
    ...
    override = getenv_compat(_WORKTREE_ROOT_ENV_VAR, "").strip()
    ...
    if not use_worktree_isolation():
        if reuse and _is_registered(worktree_path(run_id)):
            ...
            raise WorktreeError(
                ...
                "paused under a worktree-isolated LOOP_ORCHESTRATOR_ISOLATION mode. "
    ...
    if reuse:
        ...
        if not _is_registered(path):
            ...
            raise WorktreeError(
                ...
                "the run was paused under LOOP_ORCHESTRATOR_ISOLATION=none, which creates no "
