...
opening any socket if `LOOP_ORCHESTRATOR_SLACK_APP_TOKEN`, `LOOP_ORCHESTRATOR_SLACK_BOT_TOKEN`,
or `LOOP_ORCHESTRATOR_SLACK_CHANNEL` is unset -- an inbound trigger surface must
...
from loop_orchestrator.core.engine import unresolved_questions
...
from loop_orchestrator.tools.slack_io.inbound import SocketModeRequestLike
...
_CHANNEL_ENV = "LOOP_ORCHESTRATOR_SLACK_CHANNEL"
...
def build_daemon_from_env() -> SlackDaemon:
    """Fails closed (raises `RuntimeError`) unless `LOOP_ORCHESTRATOR_SLACK_APP_TOKEN`,
    `LOOP_ORCHESTRATOR_SLACK_BOT_TOKEN`, and `LOOP_ORCHESTRATOR_SLACK_CHANNEL` are all
    ...
    channel = getenv_compat(_CHANNEL_ENV)
    missing = [
        name
        for name, value in (
...
def run_daemon() -> None:
    """Entry point for `loop-orchestrator slack-listen`: build from env (fails
