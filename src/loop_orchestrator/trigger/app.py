...
`LOOP_ORCHESTRATOR_WEBHOOK_SECRET` at construction and fails closed (raises) if it
...
from loop_orchestrator.trigger.parse import parse_event
...
def create_app(dispatcher: RunDispatcher | None = None) -> FastAPI:
    ...
    `LOOP_ORCHESTRATOR_WEBHOOK_SECRET` is unset, so the app never starts unsigned."""
    secret = getenv_compat("LOOP_ORCHESTRATOR_WEBHOOK_SECRET")
    if not secret:
        raise RuntimeError(
            "LOOP_ORCHESTRATOR_WEBHOOK_SECRET is not set; refusing to start the "
...
app = create_app() if getenv_compat("LOOP_ORCHESTRATOR_WEBHOOK_SECRET") else None
