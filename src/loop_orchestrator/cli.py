...
from loop_orchestrator.slack_control import run_daemon
from loop_orchestrator.tools import issue_io
from loop_orchestrator.tools.issue_io import default_issue_reader
from loop_orchestrator.tools.llm.client import LLMClient
from loop_orchestrator.tools.repo_io import resolve_repo_slug
from loop_orchestrator.tools.worktree import prune_all, worktree_run
...
@app.callback()
def main() -> None:
    """loop-orchestrator: run a named persona loop against shared State."""
...
def _report_outcome(state: State) -> None:
    ...
    if state.pending_issue is not None:
        typer.echo(
            ...
            f"then run: loop-orchestrator resume --from-issue {state.pending_issue.number}"
        )
...
def prune_worktrees(
    ...
    if run_id is not None:
        from loop_orchestrator.tools.worktree import cleanup
