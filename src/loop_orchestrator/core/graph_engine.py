...
from loop_orchestrator.core.notify import EventKind, LifecycleEvent, Notifier
...
from loop_orchestrator.tools.slack_io import build_notifier_from_env
...
_TRANSPORT_ENV = "LOOP_ORCHESTRATOR_ESCALATION_TRANSPORT"
_SLACK_TOKEN_ENV = "LOOP_ORCHESTRATOR_SLACK_BOT_TOKEN"  # noqa: S105 -- env var name, not a credential
_SLACK_CHANNEL_ENV = "LOOP_ORCHESTRATOR_SLACK_CHANNEL"
...
def build_escalation_filer_from_env() -> EscalationFiler | None:
    ...
    `LOOP_ORCHESTRATOR_ESCALATION_TRANSPORT` (unset or `issue` ⇒ the issue path,
    ...
    """
    transport = getenv_compat(_TRANSPORT_ENV, "issue")
    if transport != "slack":
        return None
    token = getenv_compat(_SLACK_TOKEN_ENV)
    channel = getenv_compat(_SLACK_CHANNEL_ENV)
    ...
    from loop_orchestrator.tools.slack_io.escalation import slack_escalation_filer
